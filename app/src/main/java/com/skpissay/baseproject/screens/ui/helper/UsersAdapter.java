...
public class UsersAdapter extends RecyclerView.Adapter{
    ...
    private static List<Products> mObjJsonPages;
    ...
    public void changeData(List<Products> pPages) {
        ...
        if (holder instanceof DataObjectHolder) {
            try {
                Picasso.with(mObjContext)
                        .load(mObjJsonPages.get(position).getProductBaseInfoV1().getImageUrls().get200x200())
            ...
            try {
                ((DataObjectHolder) holder).fullNametxt.setText(mObjJsonPages.get(position).getProductBaseInfoV1().getTitle());
            } catch (Exception e) {
                e.printStackTrace();
            }
